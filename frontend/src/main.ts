...
async function switchNetwork(net: NetworkMode) {
  ...
  toast(
    `Switched to ${label}. Please also switch your wallet to ${label} before connecting.`,
    "info",
  );
}
...
async function fundTestnetWallet() {
  ...
  try {
    ...
    const fbRes = await fetch(
      `https://friendbot.stellar.org?addr=${userAddress}`,
    );
    if (!fbRes.ok) {
    ...
    const hasTrustline = acc.balances.some(
      (b: any) =>
        b.asset_code === "USDC" && b.asset_issuer === TESTNET_USDC_ISSUER,
    );
    ...
    toast(
      "Testnet wallet funded! USDC trustline opened and tokens acquired.",
      "success",
    );
    ...
    if (msg.includes("PATH_PAYMENT") || msg.includes("path")) {
      toast(
        "USDC trustline opened but no DEX liquidity to swap. You may need to acquire USDC manually.",
        "info",
      );
    } else {
...
document
  .getElementById("disclaimer-checkbox")!
  .addEventListener("change", (e) => {
    (
      document.getElementById("disclaimer-accept") as HTMLButtonElement
...
function minHF() {
  return expertMode ? MIN_HF_EXPERT : MIN_HF_NORMAL;
}
...
function startFreshnessTimer() {
  ...
  autoRefreshInterval = setInterval(() => {
    if (userAddress && !demoMode) loadAll();
  }, 60_000);
}
...
function animateNumber(
  ...
  to: number,
  duration = 200,
  formatFn: (n: number) => string = (n) => fmt(n, 2),
) {
...
function hideTxStepper(delay = 3000) {
  _stepperTimer = setTimeout(
    () => $("tx-stepper").classList.add("hidden"),
    delay,
  );
}
...
function getPnlEntry(
  assetId: string,
  poolId: string,
): { deposit: number; timestamp: number } | null {
...
function selectPool(pool: PoolDef) {
  ...
  document
    .querySelectorAll<HTMLButtonElement>(".pool-dropdown-item")
    .forEach((btn) => {
      btn.classList.toggle("active", btn.dataset["poolId"] === pool.id);
    });
...
function buildAssetTabs() {
  ...
  assets.forEach((asset) => {
    ...
    btn.setAttribute(
      "aria-selected",
      asset.id === selectedAsset.id ? "true" : "false",
    );
...
function selectAsset(asset: AssetInfo) {
  ...
  document.querySelectorAll<HTMLButtonElement>(".asset-tab").forEach((btn) => {
    ...
    btn.setAttribute(
      "aria-selected",
      btn.dataset["assetId"] === asset.id ? "true" : "false",
    );
  });
...
function renderApyChart(
  ...
  for (let l = 1.0; l <= maxLev; l += 0.2) {
    const p = projectRates(
      rs,
      equity * l - oldSupply,
      equity * (l - 1) - oldBorrow,
    );
    steps.push({
      lev: l,
      apy: aprToApy(p.netSupplyApr * l - p.netBorrowCost * (l - 1)),
    });
  }
  if (steps.length < 2) {
    container.innerHTML = "";
  ...
  const x = (lev: number) =>
    padL + ((lev - 1.0) / (maxLev - 1.0)) * (W - padL - padR);
  const y = (apy: number) =>
    padT + (1 - (apy - minApy) / rangeApy) * (H - padT - padB);
  const points = steps
    .map((s) => `${x(s.lev).toFixed(1)},${y(s.apy).toFixed(1)}`)
    .join(" ");
  const curProj = projectRates(
    rs,
    equity * currentLev - oldSupply,
    equity * (currentLev - 1) - oldBorrow,
  );
  const curApy = aprToApy(
    curProj.netSupplyApr * currentLev -
      curProj.netBorrowCost * (currentLev - 1),
  );
  const zeroY = y(0);
...
function renderSelectedAsset() {
  ...
  $("stat-liquidity").textContent =
    `${fmt(rs.available, 0)} ${rs.asset.symbol}`;
  ...
  utilEl.className = `stat-value ${util > 0.9 ? "hf-bad" : util > 0.75 ? "hf-warn" : ""}`;

  // Utilization bar (#13)
  const utilBar = $("util-bar");
  utilBar.style.width = `${(util * 100).toFixed(1)}%`;
  utilBar.style.background =
    util > 0.9
      ? "var(--danger)"
      : util > 0.75
        ? "var(--warning)"
  ...
  renderAprLine(
    "borrow-net-cost",
    aprToApy(rs.interestBorrowApr) - rs.blndBorrowApr,
    true,
    false,
    undefined,
    true,
...
function renderPortfolioSummary() {
  ...
  for (const [assetId, pos] of positions.byAsset) {
    ...
    const cardNetApr = rs
      ? rs.netSupplyApr * pos.leverage - rs.netBorrowCost * (pos.leverage - 1)
      : 0;
...
function renderPosition() {
  ...
  $("pos-collateral").textContent =
    `${fmt(pos.collateral, 4)} ${pos.asset.symbol}`;
  ...
  if (pnl) {
    ...
    $("pos-equity").innerHTML =
      `${fmt(pos.equity, 4)} ${pos.asset.symbol} <span class="${pnlColor}" style="font-size:11px">(${unrealizedPnl >= 0 ? "+" : ""}${fmt(unrealizedPnl, 4)} / ${unrealizedPnl >= 0 ? "+" : ""}${fmt(pnlPct, 1)}%)</span>`;
  } else {
  ...
  barWrap.setAttribute(
    "aria-label",
    `Health factor ${isFinite(hf) ? fmt(hf, 3) : "infinity"}`,
  );
  ...
  if (rs && rs.priceUsd > 0) {
    ...
    const headroom =
      Math.max(0, effectiveCollateral - effectiveDebt) * rs.priceUsd;
    ...
    if (spreadPct <= 0) {
      ...
      if (daysLeft <= 365) {
        ...
        liqDaysEl.textContent =
          daysLeft > 3650 ? ">10 years" : `~${Math.round(daysLeft)} days`;
      }
...
function setActionCardMode(mode: "open" | "adjust", pos?: AssetPosition) {
  ...
  $("action-card-title").textContent = isAdjust
    ? "Adjust Position"
    : "Open Position";
  ...
  if (isAdjust && pos) {
    ...
    $("leverage-label").innerHTML =
      'Leverage <span class="tooltip" data-tip="Multiplier on your deposit. Higher leverage amplifies both yield and liquidation risk.">?</span>';
...
function switchAdjustSubTab(sub: "leverage" | "add-funds") {
  ...
  if (isAddFunds) {
    ...
    $("leverage-label").innerHTML =
      'Leverage <span class="tooltip" data-tip="Leverage for the new deposit. This deposit is added on top of your existing position.">?</span>';
    ...
    if (userAddress) {
      fetchAssetBalance(userAddress, selectedAsset.id)
        .then((bal) => {
          $("add-funds-balance").textContent =
            `${fmt(bal, 4)} ${selectedAsset.symbol}`;
        })
        .catch(() => {});
    }
    initTooltips();
...
function updatePreview() {
  ...
  const equity =
    actionMode === "adjust" && pos
      ? pos.equity
      : actionMode === "add-funds"
        ? parseFloat(($("add-funds-input") as HTMLInputElement).value) || 0
  ...
  $("prev-hf").textContent = isFinite(hf)
    ? fmt(hf, expertMode ? 5 : 4)
    : "\u221E";
  $("prev-hf").className =
    hf > 1.1 ? "hf-ok" : hf > 1.03 ? "hf-warn" : "hf-bad";
  ...
  if (rs && rs.priceUsd > 0) {
    ...
    $("prev-headroom").className =
      headroom < 5 ? "hf-bad" : headroom < 20 ? "hf-warn" : "";
  } else {
  ...
  if (rs) {
    ...
    $("prev-net-apr").className =
      `prev-net-apr ${netApy > 0 ? "apr-great" : "apr-bad"}`;
    const prevTip = $("prev-net-tip");
    if (prevTip)
      prevTip.setAttribute(
        "data-tip",
    ...
    if (spreadPct <= 0) {
      ...
      prevLiqEl.textContent =
        days > 3650 ? ">10 years" : `~${Math.round(days)} days`;
      prevLiqEl.className =
        days < 30 ? "hf-bad" : days < 90 ? "hf-warn" : "hf-ok";
    } else {
      prevLiqEl.textContent = "\u2014";
      prevLiqEl.className = "";
    }
  ...
  // Risk zone labels (#9)
  const maxSlider =
    parseFloat(($("leverage-slider") as HTMLInputElement).max) || 10;
  ...
  if (actionMode === "open" || actionMode === "add-funds") {
    ...
    const poolAvailAfterDeposit =
      (rs?.available ?? 0) + initial * (rs ? rs.asset.maxUtil : 0.95);
  ...
  ($("hf-warning") as HTMLElement).classList.toggle(
    "hidden",
    hf >= minHF() || selectedPool.status !== 1,
  );
  ...
  if (actionMode === "add-funds") {
    ...
    ($("add-funds-btn") as HTMLButtonElement).textContent =
      addAmt > 0
...
async function loadAll() {
  ...
  try {
    ...
    const blnd = await fetchPoolPendingBlnd(
      selectedPool,
      userAddress,
      positions,
    );
...
async function openPosition() {
  ...
  if (selectedPool.status !== 1) {
    toast("Pool is frozen \u2014 cannot open new positions", "error");
    return;
  }
  ...
  if (hfForLeverage(leverage, liveAsset.cFactor, rs?.lFactor ?? 1) < minHF()) {
    toast("HF too low \u2014 reduce leverage", "error");
    return;
  }
  ...
  if (rs && firstBorrow > poolAvailAfterDeposit) {
    toast(
      `First borrow step (${fmt(firstBorrow, 0)}) exceeds pool available after deposit (${fmt(poolAvailAfterDeposit, 0)} ${rs.asset.symbol}). Reduce leverage.`,
      "error",
    );
    return;
  }
  ...
  try {
    const approveXdr = await buildApproveXdr(
      selectedPool,
      userAddress,
      liveAsset.id,
      initialStroops + 1n,
    ...
    const submitXdr = await buildOpenPositionXdr(
      selectedPool,
      userAddress,
      liveAsset,
      initialStroops,
...
async function closePosition() {
  ...
  try {
    ...
    if (
      (msg.includes("#1207") || msg.includes("InvalidUtilRate")) &&
      pos.dTokens > 0n
    ) {
      try {
        toast(
          "Pool utilization high \u2014 closing in two steps\u2026",
          "info",
        );
        ...
        const withdrawXdr = await buildWithdrawXdr(
          selectedPool,
          userAddress,
          pos.asset.id,
        );
        await signAndSubmit(
          withdrawXdr,
          `Withdraw ${selectedAsset.symbol} collateral`,
          1,
        );
        hideTxStepper();
        removePnlEntry(selectedAsset.id, selectedPool.id);
    ...
    if (msg.includes("#1207") || msg.includes("InvalidUtilRate")) {
      toast(
        "Pool utilization too high \u2014 not enough liquidity to close. Try again later.",
        "error",
      );
    } else {
...
async function maxDeposit() {
  ...
  try {
    const bal = await fetchAssetBalance(userAddress, selectedAsset.id);
    ($("initial-input") as HTMLInputElement).value = String(
      Math.floor(bal * 1e7) / 1e7,
    );
    updatePreview();
  } catch {
    /* ignore */
...
async function adjustLeverage() {
  ...
  try {
    if (targetLev > curLev) {
      const xdr = await buildIncreaseLeverageXdr(
        selectedPool,
        userAddress,
        liveAsset,
        pos,
      ...
      await signAndSubmit(
        xdr,
        `Increase leverage to ${targetLev.toFixed(1)}\u00D7`,
        0,
      );
    } else {
      const xdr = await buildDecreaseLeverageXdr(
        selectedPool,
        userAddress,
        liveAsset,
        pos,
      ...
      await signAndSubmit(
        xdr,
        `Decrease leverage to ${targetLev.toFixed(1)}\u00D7`,
        0,
      );
    }
    hideTxStepper();
    ...
    if (msg.includes("#1205") || msg.includes("InvalidHf")) {
      ...
      toast(
        "Pool utilization limit reached — not enough liquidity. Reduce target leverage.",
        "error",
      );
    } else {
      toast(msg.slice(0, 200), "error");
...
async function addFundsToPosition() {
  ...
  try {
    ...
    await signAndSubmit(
      submitXdr,
      `Add ${fmt(additional, 2)} ${liveAsset.symbol} at ${leverage.toFixed(1)}\u00D7`,
      1,
    );
    hideTxStepper();
    ...
    if (msg.includes("#1205") || msg.includes("InvalidHf")) {
      ...
      toast(
        "Pool utilization limit reached \u2014 not enough liquidity. Reduce leverage or deposit.",
        "error",
      );
    } else {
      toast(msg.slice(0, 200), "error");
...
async function resupply() {
  ...
  const bal = await fetchAssetBalance(userAddress, selectedAsset.id);
  if (bal <= 0) {
    toast(`No ${selectedAsset.symbol} in wallet to resupply`, "error");
    return;
  }
  ...
  try {
    ...
    const supplyXdr = await buildResupplyXdr(
      selectedPool,
      userAddress,
      selectedAsset.id,
      amountStroops,
    ...
    await signAndSubmit(
      supplyXdr,
      `Resupply ${fmt(bal, 4)} ${selectedAsset.symbol}`,
      1,
    );
    hideTxStepper();
    await loadAll();
...
async function claimAndConvert() {
  ...
  try {
    ...
    toast(
      `Swapping ${fmt(blndBalance, 2)} BLND \u2192 ${selectedAsset.symbol}\u2026`,
      "info",
    );
    ...
    updateTxStep(1, "done");
    toast(
      `Converted ${fmt(blndBalance, 2)} BLND \u2192 ~${estimate} ${selectedAsset.symbol}`,
      "success",
    );
    addTxToHistory(
      `Swap BLND \u2192 ${selectedAsset.symbol}`,
      swapHash,
      "success",
    );
    hideTxStepper();
...
async function connect() {
  try {
    const result = await StellarWalletsKit.authModal({
      network:
        getActiveNetwork() === "testnet" ? Networks.TESTNET : Networks.PUBLIC,
    });
...
async function switchWallet() {
  try {
    ...
    if (e?.message !== "User closed the modal")
      toast("Failed to switch wallet", "error");
  }
}
...
function switchView(view: AppView) {
  ...
  document
    .getElementById("mobile-proto-overview")
    ?.classList.toggle("active", view === "overview");
  document
    .getElementById("mobile-proto-blend")
    ?.classList.toggle("active", view === "leverage");
  document
    .getElementById("mobile-proto-swap")
    ?.classList.toggle("active", view === "swap");
  document
    .getElementById("mobile-proto-vault")
    ?.classList.toggle("active", view === "vault");
...
const SWAP_ASSETS: { symbol: string; brokerId: string }[] = [
  ...
  {
    symbol: "USDC",
    brokerId: "USDC-GA5ZSEJYB37JRC5AVCIA5MOP4RHTM335X2KGX3IHOJAPP5RE34K4KZVN",
  },
  {
    symbol: "EURC",
    brokerId: "EURC-GDHU6WRG4IEQXM5NZ4BMPKOXHW76MZM4Y2IEMFDVXBSDP6SJY4ITNPP2",
  },
  {
    symbol: "AQUA",
    brokerId: "AQUA-GBNZILSTVQZ4R7IKQDGHYGY2QXL5QOFJYQMXPKWRRM5PAV7Y4M67AQUA",
  },
  {
    symbol: "BLND",
    brokerId: "BLND-GDJEHTBE6ZHUXSWFI642DCGLUOECLHPF3KSXHPXTSTJ7E3JF6MQ5EZYY",
  },
  {
    symbol: "yXLM",
    brokerId: "yXLM-GARDNV3Q7YGT4AKSDF25LT32YSCCW4EV22Y2TV3I2PU2MMXJTEDL5T55",
  },
  {
    symbol: "USDGLO",
    brokerId: "USDGLO-GBBS25EGYQPGEZCGCFBKG4OAGFXU6DSOQBGTHELLJT3HZXZJ34HWS6XV",
  },
];
...
async function fetchSwapQuote() {
  ...
  try {
    ...
    if (quote.status === "success" && quote.estimatedBuyingAmount) {
      ...
      ($("swap-buy-amount") as HTMLInputElement).value = parseFloat(
        quote.estimatedBuyingAmount,
      ).toFixed(7);

      const sellNum = parseFloat(sellAmount);
      const buyNum = parseFloat(quote.estimatedBuyingAmount);
      const sellSym = ($("swap-sell-asset") as HTMLSelectElement)
        .selectedOptions[0].text;
      const buySym = ($("swap-buy-asset") as HTMLSelectElement)
        .selectedOptions[0].text;

      $("swap-rate").textContent =
        `1 ${sellSym} \u2248 ${(buyNum / sellNum).toFixed(6)} ${buySym}`;
      $("swap-direct").textContent = quote.directTrade
        ? `${parseFloat(quote.directTrade.buying).toFixed(7)} ${buySym}`
        : "\u2014";
      $("swap-profit").textContent = quote.profit
        ? `${quote.profit}`
        : "\u2014";
      $("swap-quote-details").classList.remove("hidden");
    } else {
      ($("swap-buy-amount") as HTMLInputElement).value =
        quote.status === "unfeasible" ? "No route" : "\u2014";
      $("swap-quote-details").classList.add("hidden");
      _lastQuote = null;
    }
  } catch (e: any) {
...
function initTooltips() {
  ...
  document
    .querySelectorAll<HTMLElement>("[data-tip]:not(.tooltip)")
    .forEach((el) => {
      el.removeAttribute("title");
...
document
  .getElementById("mobile-expert-toggle")
  ?.addEventListener("click", toggleExpert);
...
$("network-toggle").addEventListener("click", () => {
  const next: NetworkMode =
    getActiveNetwork() === "mainnet" ? "testnet" : "mainnet";
  switchNetwork(next);
});
...
document
  .getElementById("mobile-proto-blend")
  ?.addEventListener("click", () => switchView("leverage"));
document
  .getElementById("mobile-proto-swap")
  ?.addEventListener("click", () => switchView("swap"));
document
  .getElementById("mobile-proto-vault")
  ?.addEventListener("click", () => switchView("vault"));
...
document
  .querySelectorAll<HTMLButtonElement>(".mobile-card-tab")
  .forEach((btn) => {
    btn.addEventListener("click", () => {
      document
        .querySelectorAll(".mobile-card-tab")
...
document.querySelectorAll<HTMLButtonElement>(".vault-tab").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    $("vault-withdraw-section").classList.toggle(
      "hidden",
      action !== "withdraw",
    );
  });
...
document.querySelectorAll(".slippage-opt").forEach((btn) => {
  btn.addEventListener("click", () => {
    document
      .querySelectorAll(".slippage-opt")
      .forEach((b) => b.classList.remove("active"));
    btn.classList.add("active");
    (
      document.getElementById("slippage-custom-input") as HTMLInputElement
    ).value = "";
...
$("slippage-custom-input").addEventListener("input", () => {
  const val = parseFloat(
    ($("slippage-custom-input") as HTMLInputElement).value,
  );
  if (val > 0 && val <= 50) {
    document
      .querySelectorAll(".slippage-opt")
...
$("swap-sell-asset").addEventListener("change", () => {
  _lastQuote = null;
  debounceQuote();
  updateSwapBalance();
});
$("swap-buy-asset").addEventListener("change", () => {
  _lastQuote = null;
  debounceQuote();
});
...
async function updateSwapBalance() {
  ...
  if (!contractId) {
    $("swap-sell-balance").textContent = "\u2014";
    return;
  }
  try {
    const bal = await fetchAssetBalance(userAddress, contractId);
    const sym = ($("swap-sell-asset") as HTMLSelectElement).selectedOptions[0]
      .text;
...
$("add-funds-max-btn").addEventListener("click", async () => {
  ...
  try {
    const bal = await fetchAssetBalance(userAddress, selectedAsset.id);
    ($("add-funds-input") as HTMLInputElement).value = String(
      Math.floor(bal * 1e7) / 1e7,
    );
    updatePreview();
  } catch {
...
($("leverage-slider") as HTMLInputElement).addEventListener(
  "input",
  updatePreview,
);
...
($("initial-input") as HTMLInputElement).addEventListener("input", () => {
  refreshTabData();
  updatePreview();
});
($("initial-input") as HTMLInputElement).addEventListener("change", () => {
  refreshTabData();
  updatePreview();
});
...
$("demo-btn").addEventListener("click", () => {
  ...
  reserves = assets.map((a) => ({
    ...
    cFactor: a.cFactor,
    lFactor: 1,
    interestSupplyApr: 4.2,
    interestBorrowApr: 6.8,
    blndSupplyApr: 2.1,
    blndBorrowApr: 1.5,
    netSupplyApr: 6.3,
    netBorrowCost: 5.3,
  ...
  positions.byAsset.set(sampleAsset.id, {
    ...
    leverage: 2.5,
    hf: 1.15,
    bTokens: 50000000000n,
    dTokens: 30000000000n,
  } as AssetPosition);
  buildPoolTabs();
  buildAssetTabs();
  renderPoolFooter();
 ...
 * Only covers static text nodes that are never overwritten by JS logic.
 * Dynamic strings (toasts, button labels set in code) use t() at call-site.
 */
const I18N_TEXT_MAP: Record<string, string> = {
  // Disclaimer modal
  "disclaimer-title":    "disclaimer.title",
  "disclaimer-accept":   "disclaimer.enterApp",
  // Top nav
  "proto-overview":      "nav.dashboard",
  "proto-vault":         "nav.vault",
  "proto-swap":          "nav.swap",
  "connect-btn":         "nav.connectWallet",
  "disconnect-btn":      "nav.disconnect",
  "switch-wallet-btn":   "nav.switch",
  // Mobile sidebar nav
  "mobile-proto-overview": "nav.dashboard",
  "mobile-proto-vault":    "nav.vault",
  "mobile-proto-swap":     "nav.swap",
  // Testnet banner
  "fund-testnet-btn":    "testnet.fundWallet",
  // Refresh
  "refresh-btn":         "misc.refresh",
};

/** data-tip attributes to translate (element-id → key) */
const I18N_TIP_MAP: Record<string, string> = {
  "network-toggle":      "misc.switchNetwork",
  "switch-wallet-btn":   "misc.switchAccount",
  "alert-bell-btn":      "misc.setAlerts",
  "mobile-expert-toggle": "misc.expertModeTip",
  "mobile-theme-toggle": "misc.themeTip",
  "fund-testnet-btn":    "testnet.fundWalletTip",
};
...
function applyI18nToDOM(): void {
  ...
  for (const [id, key] of Object.entries(I18N_TEXT_MAP)) {
    const el = document.getElementById(id);
    if (!el) continue;
    // Preserve child elements (e.g. caret spans inside nav buttons)
  ...
  for (const [id, key] of Object.entries(I18N_TIP_MAP)) {
    const el = document.getElementById(id);
    if (el) el.setAttribute("data-tip", t(key));
  }

  // Disclaimer body (contains HTML)
  const discBody = document.getElementById("disclaimer-body");
  if (discBody) {
    discBody.innerHTML = `<p>${t("disclaimer.body1")}</p>
      <p>${t("disclaimer.body2")}</p>
      <ul>
        <li>${t("disclaimer.li1")}</li>
        <li>${t("disclaimer.li2")}</li>
        <li>${t("disclaimer.li3")}</li>
        <li>${t("disclaimer.li4")}</li>
      </ul>`;
  }
  const discCheck = document.querySelector<HTMLElement>(".disclaimer-check span");
  if (discCheck) discCheck.textContent = t("disclaimer.checkLabel");

  // Testnet banner text
  const testnetHint = document.getElementById("testnet-hint");
  if (testnetHint) testnetHint.textContent = t("testnet.hint");

  // Re-init tooltips so new data-tip values are picked up
  initTooltips();
}
...
if (langSelect) {
  langSelect.value = getSavedLocale();
  langSelect.addEventListener("change", async () => {
    const locale = langSelect.value as Locale;
    await setLocale(locale);
    applyI18nToDOM();
    // Re-render dynamic content that uses t() at call-site
    buildPoolTabs();
    buildAssetTabs();
    renderPoolFooter();
    updatePreview();
...
function renderOverview(
  ...
  $("ov-total-equity").textContent =
    totalEquity > 0 ? `$${fmt(totalEquity, 2)}` : "--";
  $("ov-total-debt").textContent =
    totalDebt > 0 ? `$${fmt(totalDebt, 2)}` : "--";
  ...
  if (blendPos.length > 0) {
    ...
    for (const bp of blendPos) {
      const rs = bp.reserves.find((r) => r.asset.id === bp.asset.id);
      const price = rs?.priceUsd ?? 0;
      const batchNetApr = rs
        ? rs.netSupplyApr * bp.pos.leverage -
          rs.netBorrowCost * (bp.pos.leverage - 1)
        : 0;
      const netApy = aprToApy(batchNetApr);
      const hfColor =
        bp.pos.hf > 1.1 ? "hf-ok" : bp.pos.hf > 1.03 ? "hf-warn" : "hf-bad";
  ...
  if (vaultPos.length > 0) {
    ...
    for (const vp of vaultPos) {
      const hf = vp.stats
        ? formatHf(vp.stats.healthFactor)
        : { text: "--", cls: "" };
  ...
  container
    .querySelectorAll<HTMLElement>("tr[data-nav-pool]")
    .forEach((row) => {
  ...
  container
    .querySelectorAll<HTMLElement>(".overview-vault-card")
    .forEach((card) => {
...
async function refreshVaultView() {
  ...
  if (vaultReady) {
    linkEl.textContent =
      vault.vaultId.slice(0, 8) + "..." + vault.vaultId.slice(-4);
  ...
  if (stats) {
    ...
    if (stats.netApy !== null) {
      ...
      apyEl.textContent =
        (vaultApy >= 0 ? "+" : "") + vaultApy.toFixed(2) + "%";
      apyEl.className =
        "stat-value mono " + (vaultApy > 0 ? "hf-ok" : "hf-bad");
      const vaultTip = $("vault-apy-tip");
      if (vaultTip)
        vaultTip.setAttribute(
          "data-tip",
    ...
    $("vault-collateral").textContent =
      fmt(stats.collateralValue, 2) + " " + vault.assetSymbol;
    $("vault-debt").textContent =
      fmt(stats.debtValue, 2) + " " + vault.assetSymbol;
    $("vault-equity").textContent =
      fmt(stats.totalEquity, 2) + " " + vault.assetSymbol;

    if (stats.supplyApr !== null) {
      $("vault-supply-apr").textContent =
        "+" + aprToApy(stats.supplyApr).toFixed(2) + "%";
      $("vault-supply-apr").className = "metric-value mono hf-ok";
    }
    if (stats.borrowApr !== null) {
      $("vault-borrow-apr").textContent =
        "-" + aprToApy(stats.borrowApr).toFixed(2) + "%";
    ...
    $("vault-hf-bar-label").textContent = isFinite(stats.healthFactor)
      ? stats.healthFactor.toFixed(3)
      : "\u221e";

    // Rebalance button — enabled only when HF < min_hf
    const needsRebalance =
      isFinite(stats.healthFactor) && stats.healthFactor < vault.minHf;
  ...
  if (connected && userAddress) {
    ...
    if (pos && pos.underlyingValue > 0) {
      ...
      $("vault-withdraw-balance").textContent =
        pos.underlyingValue.toFixed(4) + " " + vault.assetSymbol;
      // Share of vault
...
$("vault-withdraw-btn").addEventListener("click", async () => {
  ...
  try {
    ...
    ($("vault-withdraw-btn") as HTMLButtonElement).textContent =
      "Withdrawing...";
...
$("vault-rebalance-btn").addEventListener("click", async () => {
  ...
  try {
    ...
    ($("vault-rebalance-btn") as HTMLButtonElement).textContent =
      "Rebalancing...";
...
(async () => {
  // Initialize i18n first so all t() calls resolve correctly
  await initI18n();
  applyI18nToDOM();
...
$("alert-bell-btn").addEventListener("click", () => {
  ...
  const curLev =
    parseFloat(($("leverage-slider") as HTMLInputElement).value) || 5;
  const brackets = [2, 3, 5, 8, 10];
  const closest = brackets.reduce((a, b) =>
    Math.abs(b - curLev) < Math.abs(a - curLev) ? b : a,
  );
...
$("alert-subscribe-btn").addEventListener("click", async () => {
  ...
  const leverageBracket = Number(
    ($("alert-leverage") as HTMLSelectElement).value,
  );
